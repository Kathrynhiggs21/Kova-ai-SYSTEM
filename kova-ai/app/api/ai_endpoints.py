...
async def analyze_repository(command: ClaudeCommand) -> ClaudeResponse:
    ...
    return ClaudeResponse(status="success", data=analysis, repository_info=repo_data)
...
def require_github_token() -> str:
    """Return a configured GitHub token after normalizing surrounding whitespace."""
...
async def fetch_repository_data(repository: str, github_token: str) -> Dict[str, Any]:
    ...
    async with httpx.AsyncClient() as client:
        ...
        contents_response = await client.get(
            f"https://api.github.com/repos/{repository}/contents", headers=headers
        )
        contents_data = (
            contents_response.json() if contents_response.status_code == 200 else []
        )
...
async def get_repository_structure(
    repository: str, github_token: str
) -> Dict[str, Any]:
...
async def get_file_content(
    repository: str, file_path: str, github_token: str
) -> Dict[str, Any]:
